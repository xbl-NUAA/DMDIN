...
class DMDR(nn.Module):
    def __init__(self, channels_list, num_sample, class_list, weight_c = 1e-2):
        ...
        for i in range(len(class_list)):
            ...
            index += class_list[i]
        self.block = (mask_block + torch.eye(sum(class_list))).to(self.device)

    def forward(self, x_list):
        f_list = []
        zf_list = []
        for x, model in zip(x_list, self.model_list):
            out_f = model(x.float().to(self.device))
